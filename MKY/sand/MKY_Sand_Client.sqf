...
MKY_fnc_set_Object_Direction = {
	...
	scriptSetObjDir = [] spawn {
		...
		waitUntil {sleep 0.25;!(isNil "objSand")};
		while {true} do {
			sleep 0.25;
			if (isNil "objSand") exitWith {true;};
			/*
...
MKY_fnc_sand_Init = {
	...
	objSand = "#particlesource" createVehicleLocal (getPosATL player);
	/* objSand attachTo [objEmitterHost,[0,0,0]];*/

	// create the children emitters that will follow the parent emitter
	objSandN = "#particlesource" createVehicleLocal (getPosATL player);
	objSandS = "#particlesource" createVehicleLocal (getPosATL player);
	objSandE = "#particlesource" createVehicleLocal (getPosATL player);
	objSandW = "#particlesource" createVehicleLocal (getPosATL player);
	...
	while {true} do {
		...
		while {varEnableSand} do {
			...
			if !(isNil "objSand") then {
				{
					_x setPosATL [getPosATL player select 0, getPosATL player select 1, 0];
		...
		*/
		...
		while {varEnableSand} do {
			...
			if (_cnt > _rnd) then {
				...
				sand_Array set [5,[0,150,0]]; // North is in front, 150 meter out
				objSandN setParticleParams sand_Array;
				sand_Array set [5,[0,-150,0]]; // south is behind, only 30 meter out
				objSandS setParticleParams sand_Array;
				sand_Array set [5,[150,0,0]]; // East is to right, 70 meter out
				objSandE setParticleParams sand_Array;
				sand_Array set [5,[-150,0,0]]; // West is to left, 70 meter out
				objSandW setParticleParams sand_Array;
				_cnt = 0;
				_rnd = ([10,25] call BIS_fnc_randomNum);
			};
